...
@section('action-content')
  ...
  <div class="box-header">
    <div class="row">
        <div class="col-sm-8">
          <h3 class="box-title"><b style="color: #AA0000">Danh sách tài khoản</b></h3>
        </div>
        <div class="col-sm-4">
          <a style="float: right;" class="btn btn-primary" href="{{ route('user-management.create') }}">Thêm mới</a>
          <a style="float: right" href="javascript:history.back()" class="btn btn-danger">Quay Lại</a>
        </div>
    </div>
  </div>
  <!-- /.box-header -->
          ...
          <table id="example2" class="table table-bordered table-hover dataTable" role="grid" aria-describedby="example2_info">
            ...
            @foreach ($users as $user)
                <tr role="row" class="odd">
                  ...
                  <td>
                    <form class="row" method="POST" action="{{ route('user-management.destroy', ['id' => $user->id]) }}" onsubmit = "return confirm('Bạn có chắc ! muốn xóa tài khoản này?')">
